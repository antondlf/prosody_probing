"""From https://github.com/luferrer/ConfidenceIntervals/blob/main/confidence_intervals/confidence_intervals.py"""
...
def metric_wrapper(labels, samples, samples2, metric_name, indices=None):
    ...
    metric = r2_score if metric_name == 'R2' else f1_score
    ...
    if labels is not None:
        if samples2 is not None:
            ...
            if metric_name == 'f1_macro':
                return metric(l, s, average='macro')
            elif metric_name == 'f1_score':
                pos_label = l.unique().min()
                return metric(l, s, pos_label=pos_label)
    else:
        if samples2 is not None:
            return metric(s, s2)
        else:
